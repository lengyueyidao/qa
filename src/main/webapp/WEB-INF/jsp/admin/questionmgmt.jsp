                    ...
                    <table class="table table-hover" id="dataTables-example">
                        ...
                        <c:forEach items="${questions}" var="question">
                            <tr>
                                ...
                                <td style="text-align:center;"><c:if test="${question.isover == 0}"><a onclick="overQuestion('${question.id}',1)">解决</a></c:if><c:if test="${question.isover == 1}"><a onclick="overQuestion('${question.id}',0)">未解决</a></c:if> | <c:if test="${question.isover == 0}"><a onclick="del('${question.id}', 1)">删除</a></c:if><c:if test="${question.isover == 1}"><a onclick="del('${question.id}', 0)">删除</a></c:if></td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    </table>
                            ...
                            <c:when test="${page.totalCount!=0}">
                                <c:choose>
                                    <c:when test="${page.hasFirst}">
                                        <li><a href="adminquestion?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=1" >首页</a></li>
                                    </c:when>
                                </c:choose>

                                <c:choose>
                                    <c:when test="${page.hasPre}">
                                        <li><a href="adminquestion?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=${page.pageNow-1}">上一页</a></li>
                                    </c:when>
                                </c:choose>
                                ...
                                <c:forEach var="i" begin="${(page.pageNow-1)<=0?1:((page.pageNow+2)>page.totalPageCount?((page.pageNow-2)<=0?1:page.pageNow-2):(page.pageNow-2)<=0?1:page.pageNow-2)}" end="${(page.pageNow-1)<=0?((page.pageNow+2)>page.totalPageCount?page.totalPageCount:page.pageNow+2):((page.pageNow+2)>page.totalPageCount?page.totalPageCount:((page.pageNow+2)>page.totalPageCount)?page.totalPageCount:(page.pageNow+2))}" step="1">
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <li><a href="adminquestion?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=${i}">${i}</a></li>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
                                ...
                                <c:choose>
                                    <c:when test="${page.hasNext}">
                                        <li><a href="adminquestion?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=${page.pageNow+1}">下一页</a></li>
                                    </c:when>
                                </c:choose>

                                <c:choose>
                                    <c:when test="${page.hasLast}">
                                        <li><a href="adminquestion?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=${page.totalPageCount}">尾页</a></li>
                                    </c:when>
                                </c:choose>
...
<script>
    ...
    function del(qid, isover) {
        ...
        $.post("ucenterquestiondel", {qid:qid}, function(data) {
            if(data.status == "ok") {
                ...
                window.location.href="adminquestion?q=${q}&pageNow=${page.pageNow}";
            }
            else {
                alert(data.msg);
            }
    ...
    // 搜索
    function search() {
        window.location.href="adminquestion?q="+$('#q').val();
    }
</script>
