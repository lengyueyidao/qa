								...
								<c:when test="${page.totalCount!=0}">
									<c:choose>
										<c:when test="${page.hasFirst}">
											<a class="btn" href="solve?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=1" >首页</a>
										</c:when>
									</c:choose>

									<c:choose>
										<c:when test="${page.hasPre}">
											<a class="btn" href="solve?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=${page.pageNow-1}">上一页</a>
										</c:when>
									</c:choose>
									...
									<c:forEach var="i" begin="${(page.pageNow-1)<=0?1:((page.pageNow+2)>page.totalPageCount?((page.pageNow-2)<=0?1:page.pageNow-2):(page.pageNow-2)<=0?1:page.pageNow-2)}" end="${(page.pageNow-1)<=0?((page.pageNow+2)>page.totalPageCount?page.totalPageCount:page.pageNow+2):((page.pageNow+2)>page.totalPageCount?page.totalPageCount:((page.pageNow+2)>page.totalPageCount)?page.totalPageCount:(page.pageNow+2))}" step="1">
										<c:choose>
											...
											<c:otherwise>
												<a class="btn" href="solve?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=${i}">${i}</a>
											</c:otherwise>
										</c:choose>
									</c:forEach>
									...
									<c:choose>
										<c:when test="${page.hasNext}">
											<a class="btn" href="solve?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=${page.pageNow+1}">下一页</a>
										</c:when>
									</c:choose>

									<c:choose>
										<c:when test="${page.hasLast}">
											<a class="btn" href="solve?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=${page.totalPageCount}">尾页</a>
										</c:when>
									</c:choose>
