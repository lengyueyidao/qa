	...
	<body style="background-color: #F5F5D5;">
		...
		<div class="search-area-wrapper">
            <div class="search-area container">
                <h3 class="search-header">标签太多了？</h3>
                <p class="search-tag-line">你不妨试一下搜索哦，说不准就找到了呢！</p>
                <form id="search-form" class="search-form clearfix" method="get" action="tags" autocomplete="off">
                    <input class="search-term" type="text" id="q" name="q" value="${q}" placeholder="在这儿输入你要查找的标签" title="* 请输入你要查找的标签！" />
							...
							<div class="each-post " >
								...
								<ul class="tags">
									...
									<c:forEach items="${tags}" var="tag">
										<li><a href="questiontag?tagid=${tag.id}">${tag.name} - ${tag.questionCount}</a></li>
									</c:forEach>
								</ul>
							</div>
									...
									<c:when test="${page.totalCount!=0}">
										<c:choose>
											<c:when test="${page.hasFirst}">
												<a class="btn" href="tags?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=1" >首页</a>
											</c:when>
										</c:choose>

										<c:choose>
											<c:when test="${page.hasPre}">
												<a class="btn" href="tags?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=${page.pageNow-1}">上一页</a>
											</c:when>
										</c:choose>
										...
										<c:forEach var="i" begin="${(page.pageNow-1)<=0?1:((page.pageNow+2)>page.totalPageCount?((page.pageNow-2)<=0?1:page.pageNow-2):(page.pageNow-2)<=0?1:page.pageNow-2)}" end="${(page.pageNow-1)<=0?((page.pageNow+2)>page.totalPageCount?page.totalPageCount:page.pageNow+2):((page.pageNow+2)>page.totalPageCount?page.totalPageCount:((page.pageNow+2)>page.totalPageCount)?page.totalPageCount:(page.pageNow+2))}" step="1">
											<c:choose>
												...
												<c:otherwise>
													<a class="btn" href="tags?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=${i}">${i}</a>
												</c:otherwise>
											</c:choose>
										</c:forEach>
										...
										<c:choose>
											<c:when test="${page.hasNext}">
												<a class="btn" href="tags?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=${page.pageNow+1}">下一页</a>
											</c:when>
										</c:choose>

										<c:choose>
											<c:when test="${page.hasLast}">
												<a class="btn" href="tags?<c:if test="${not empty q}">q=${q}&</c:if>pageNow=${page.totalPageCount}">尾页</a>
											</c:when>
										</c:choose>
